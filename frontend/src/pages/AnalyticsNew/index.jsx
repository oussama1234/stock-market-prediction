...
const NavigationBar = memo(({ symbol, lastUpdate, formatTime, refreshing, onRefresh }) => (
  <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 mb-4 sm:mb-6">
    <Link 
      to={`/stock/${symbol}`} 
      className="group inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-md hover:shadow-lg text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-semibold transition-all hover:scale-105 text-sm sm:text-base w-fit"
    >
      <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5 group-hover:-translate-x-1 transition-transform" />
      <span className="hidden xs:inline">Back to Stock Details</span>
    ...
    <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2 md:gap-3">
      {lastUpdate && (
        <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-md">
          ...
          <span className="text-xs font-medium text-gray-600 dark:text-gray-300">Updated {formatTime()}</span>
        </div>
      )}
      <button 
        onClick={onRefresh} 
        disabled={refreshing} 
        className="group inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all hover:scale-105 disabled:opacity-50 shadow-lg hover:shadow-xl text-sm sm:text-base"
      >
...
const StockHeader = memo(({ stock, symbol, currentPrice, change, changePct, isPositive }) => (
  <div className="group relative bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-2xl hover:shadow-3xl p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 overflow-hidden transition-all duration-500">
    ...
    <div className="relative z-10">
      <div className="flex flex-col lg:flex-row items-start justify-between gap-4 sm:gap-6">
        <div className="flex items-center gap-3 sm:gap-4 md:gap-6">
          ...
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-2">
              <BarChart3 className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-indigo-600" />
            ...
            <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-700 dark:text-gray-300 font-semibold mb-2 sm:mb-3">{stock?.name}</p>
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              <Badge icon={<LineChart className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} text="Technical Analysis" color="indigo" />
              <Badge icon={<Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} text="Live Signals" color="purple" />
              <Badge icon={<Target className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} text="S/R Levels" color="pink" />
              <Badge icon={<Radio className="w-3 h-3 sm:w-3.5 sm:h-3.5" />} text="Live" color="green" pulse />
            </div>
          </div>
        </div>
        ...
        <div className="text-center lg:text-right w-full lg:w-auto">
          ...
          <div className={`inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-base sm:text-lg md:text-xl font-bold shadow-lg backdrop-blur-md transition-all hover:scale-105 ${
            isPositive 
...
const Badge = ({ icon, text, color, pulse }) => (
  <div className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 bg-${color}-100 text-${color}-700 rounded-full backdrop-blur-sm shadow-sm hover:shadow-md transition-shadow ${pulse ? 'animate-pulse' : ''}`}>
    {icon}
...
const TradingViewChart = memo(({ chartContainerRef, symbol }) => (
  <div className="group relative bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-xl hover:shadow-2xl p-4 sm:p-6 mb-4 sm:mb-6 transition-all duration-300">
    <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
      ...
      <div>
        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Live Trading Chart</h2>
        <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Real-time price action with indicators</p>
      </div>
    </div>
    <div ref={chartContainerRef} id={`tradingview_${symbol}_${Date.now()}`} style={{ height: '500px' }} className="sm:h-[600px] md:h-[700px] lg:h-[800px] w-full rounded-xl sm:rounded-2xl overflow-hidden shadow-inner"></div>
  </div>
));

// Technical Indicators Component
const TechnicalIndicators = memo(({ indicators, currentPrice }) => (
  <div className="relative bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-xl hover:shadow-2xl p-4 sm:p-6 transition-all duration-300">
    <div className="absolute inset-0 bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 dark:from-purple-900/20 dark:via-indigo-900/20 dark:to-blue-900/20 rounded-2xl sm:rounded-3xl opacity-50"></div>
    
    <div className="relative z-10">
      <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
        <div className="p-2 sm:p-3 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-lg sm:rounded-xl shadow-lg">
          <Activity className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
        </div>
        <div>
          <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Technical Indicators</h3>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
            <span className="text-xs font-semibold text-gray-600 dark:text-gray-400">Live Analysis</span>
          </div>
        </div>
      </div>
...
const RSIIndicator = memo(({ rsi }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-gray-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-600 hover:border-purple-200 dark:hover:border-purple-500">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl ${color.bg}`}>
            <Gauge className={`w-4 h-4 sm:w-5 sm:h-5 ${color.text}`} />
          </div>
          <div>
            <div className="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-300">RSI (14)</div>
            <span className={`inline-block px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full ${color.bg} ${color.text}`}>
              {color.label}
            </span>
          </div>
        </div>
        <div className="text-2xl sm:text-3xl md:text-4xl font-black text-gray-900 dark:text-white">{rsi.toFixed(1)}</div>
      </div>
      
      <div className="relative">
        <div className="h-3 sm:h-4 bg-gradient-to-r from-green-200 via-blue-200 to-red-200 rounded-full overflow-hidden">
          <div className={`h-full ${color.bar} transition-all duration-500 shadow-lg`} style={{ width: `${rsi}%` }}></div>
        </div>
        <div className="flex justify-between text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-2 px-1">
          <span>0</span>
...
const MACDIndicator = memo(({ macd }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-gray-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-600 hover:border-purple-200 dark:hover:border-purple-500">
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl ${isBullish ? 'bg-green-100' : 'bg-red-100'}`}>
            {isBullish ? <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-green-600" /> : <TrendingDown className="w-4 h-4 sm:w-5 sm:h-5 text-red-600" />}
          </div>
          <div>
            <div className="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-300">MACD</div>
            <span className={`inline-block px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full ${isBullish ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {macd.signal?.toUpperCase() || 'NEUTRAL'}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-1 sm:gap-2">
          {isBullish ? <ChevronUp className="w-6 h-6 sm:w-8 sm:h-8 text-green-600" /> : <ChevronDown className="w-6 h-6 sm:w-8 sm:h-8 text-red-600" />}
          <span className="text-xl sm:text-2xl md:text-3xl font-black text-gray-900 dark:text-white">{macd.value.toFixed(3)}</span>
        </div>
      </div>
      
      <div className="grid grid-cols-2 gap-2 sm:gap-3 mt-3">
        <div className="text-center p-2 sm:p-3 bg-gray-50 dark:bg-gray-600 rounded-lg">
          <div className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-300 mb-1">MACD Line</div>
          <div className="text-sm sm:text-base md:text-lg font-bold text-gray-900 dark:text-white">{macd.value.toFixed(3)}</div>
        </div>
        {macd.signal_line !== undefined && (
          <div className="text-center p-2 sm:p-3 bg-gray-50 dark:bg-gray-600 rounded-lg">
            <div className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-300 mb-1">Signal Line</div>
            <div className="text-sm sm:text-base md:text-lg font-bold text-gray-900 dark:text-white">{macd.signal_line.toFixed(3)}</div>
          </div>
        )}
      </div>
      
      <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-600">
        <div className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400 mb-2">Momentum:</div>
        <div className="flex gap-0.5 sm:gap-1">
          {[...Array(10)].map((_, i) => (
            <div 
              key={i} 
              className={`flex-1 h-2 sm:h-3 rounded transition-all ${
                isBullish 
...
const MovingAverages = memo(({ ema20, ema50, currentPrice }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-gray-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-600 hover:border-purple-200 dark:hover:border-purple-500">
      <div className="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-300 mb-3 sm:mb-4 flex items-center gap-2">
        <div className="p-1.5 sm:p-2 bg-blue-100 rounded-lg">
          <TrendingUpDown className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600" />
        </div>
        <span>Moving Averages</span>
        <span className={`ml-auto px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full ${aboveEMA20 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {aboveEMA20 ? 'ABOVE EMA' : 'BELOW EMA'}
        </span>
      </div>
      
      <div className="space-y-2 sm:space-y-3">
        <div className="flex justify-between items-center p-2 sm:p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg sm:rounded-xl">
          <div>
            <div className="text-[10px] sm:text-xs text-gray-600 font-semibold">EMA 20</div>
            <div className="text-base sm:text-lg md:text-xl font-black text-blue-600">${ema20.toFixed(2)}</div>
          </div>
          <div className="text-right">
            <div className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400">Distance</div>
            <div className={`text-xs sm:text-sm font-bold ${currentPrice > ema20 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {ema50 && (
          <div className="flex justify-between items-center p-2 sm:p-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg sm:rounded-xl">
            <div>
              <div className="text-[10px] sm:text-xs text-gray-600 font-semibold">EMA 50</div>
              <div className="text-base sm:text-lg md:text-xl font-black text-purple-600">${ema50.toFixed(2)}</div>
            </div>
            <div className="text-right">
              <div className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400">Distance</div>
              <div className={`text-xs sm:text-sm font-bold ${currentPrice > ema50 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {ema50 && (
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-200 dark:border-gray-600">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400">Trend:</span>
            <span className={`px-2 sm:px-3 py-0.5 sm:py-1 text-[10px] sm:text-xs font-bold rounded-full ${trend === 'bullish' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              {trend === 'bullish' ? '▲ BULLISH' : '▼ BEARISH'}<span className="hidden sm:inline"> CROSSOVER</span>
            </span>
          </div>
        </div>
      )}
...
const SupportResistance = memo(({ supportResistance, currentPrice }) => (
  <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-xl hover:shadow-2xl p-6 transition-all duration-300">
    <div className="absolute inset-0 bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-green-900/20 dark:via-emerald-900/20 dark:to-teal-900/20 rounded-3xl opacity-50"></div>
    ...
    <div className="relative z-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Support & Resistance</h3>
          <p className="text-xs text-gray-600 dark:text-gray-400">Key price levels</p>
        </div>
      </div>
      
      <div className="space-y-4">
        <div className="bg-white dark:bg-gray-700 rounded-2xl p-4 shadow-md">
          <div className="flex items-center gap-2 text-sm font-semibold text-green-700 dark:text-green-400 mb-3">
          ...
          {supportResistance?.supports && supportResistance.supports.length > 0 ? (
            <div className="space-y-2">
              {supportResistance.supports.slice(0, 3).map((s, i) => (
                <div key={i} className="flex justify-between items-center p-3 bg-green-50 dark:bg-green-900/20 rounded-xl hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors group">
                  <div>
                    <div className="text-gray-900 dark:text-white font-bold text-lg">${(s.price || s).toFixed(2)}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Level {i + 1}</div>
                  </div>
                  <div className="text-right">
                    ...
                    {s.strength && (
                      <div className="text-xs text-gray-500 dark:text-gray-400">Strength: {s.strength}</div>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="bg-white dark:bg-gray-700 rounded-2xl p-4 shadow-md">
          <div className="flex items-center gap-2 text-sm font-semibold text-red-700 dark:text-red-400 mb-3">
          ...
          {supportResistance?.resistances && supportResistance.resistances.length > 0 ? (
            <div className="space-y-2">
              {supportResistance.resistances.slice(0, 3).map((r, i) => (
                <div key={i} className="flex justify-between items-center p-3 bg-red-50 dark:bg-red-900/20 rounded-xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors group">
                  <div>
                    <div className="text-gray-900 dark:text-white font-bold text-lg">${(r.price || r).toFixed(2)}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">Level {i + 1}</div>
                  </div>
                  <div className="text-right">
                    ...
                    {r.strength && (
                      <div className="text-xs text-gray-500 dark:text-gray-400">Strength: {r.strength}</div>
                    )}
                  </div>
                </div>
              ))}
...
const TradingSignals = memo(({ indicators, supportResistance, prediction, isBullish, confidence, currentPrice, predictedPrice, priceChangePercent, alerts }) => {
  ...
  return (
    <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-xl hover:shadow-2xl p-6 mb-6 transition-all duration-300">
      <div className="absolute inset-0 bg-gradient-to-br from-yellow-50 via-orange-50 to-amber-50 dark:from-yellow-900/20 dark:via-orange-900/20 dark:to-amber-900/20 rounded-3xl opacity-50"></div>
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Trading Signals & Alerts</h3>
              <p className="text-xs text-gray-600 dark:text-gray-400">AI-powered analysis</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
...
const SignalCard = memo(({ signal }) => {
  ...
  return (
    <div className={`group relative bg-white dark:bg-gray-700 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border-l-4 ${color.border}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{signal.title}</h4>
          <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{signal.description}</p>
        </div>
        
        <div className="ml-4 text-right">
          <div className="text-xs text-gray-600 dark:text-gray-400 mb-1">Confidence</div>
          <div className="text-3xl font-black text-gray-900 dark:text-white">{Math.round(signal.confidence * 100)}%</div>
...
const FearGreedWidget = memo(({ fearGreed }) => (
  <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-xl hover:shadow-2xl p-6 transition-all duration-300">
    <div className="absolute inset-0 bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 dark:from-pink-900/20 dark:via-rose-900/20 dark:to-red-900/20 rounded-3xl opacity-50"></div>
    ...
    <div className="relative z-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Fear & Greed Index</h3>
          <p className="text-xs text-gray-600 dark:text-gray-400">Market sentiment indicator</p>
        </div>
      </div>
      
      <div className="bg-white dark:bg-gray-700 rounded-2xl p-8 text-center shadow-md">
        <div className="text-7xl font-black mb-3 bg-gradient-to-r from-pink-600 to-rose-600 bg-clip-text text-transparent">
          {fearGreed.value}
        </div>
        <div className="text-2xl font-bold text-gray-900 dark:text-white mb-3">{fearGreed.classification}</div>
        <p className="text-sm text-gray-600 dark:text-gray-400">{fearGreed.description}</p>
      </div>
    </div>
  </div>
));
...
const EmptyState = ({ icon, title, text }) => (
  <div className="text-center py-8">
    ...
    {title && <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{title}</h4>}
