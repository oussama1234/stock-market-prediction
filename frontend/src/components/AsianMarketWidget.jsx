...
import { 
  TrendingUp, TrendingDown, Minus, RefreshCw, Globe, 
  Zap, BarChart3, Activity, ArrowUpRight, ArrowDownRight
} from 'lucide-react';
import { marketAPI } from '../services/api';

/**
 * Enhanced Asian Market Widget - Modern, animated, performant
 * 
 * Features:
 ...
 */
export default function AsianMarketWidgetEnhanced({ compact = false }) {
  const { data, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ['asian-markets'],
    queryFn: async () => await marketAPI.getAsianMarkets(),
    ...
    <div className="relative group">
      ...
      <div className="relative bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/50 overflow-hidden">
        ...
        <div className="relative p-6">
          ...
          {/* Asian Influence Meter */}
          {meta.asian_influence_score !== undefined && (
    ...
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center gap-3">
        <div className="relative">
          <Globe className="w-7 h-7 text-indigo-600 animate-pulse" />
...
const InfluenceMeterEnhanced = memo(({ score, impactPercent, avgChange }) => {
  ...
  const theme = useMemo(() => ({
    gradient: isBullish 
      ? 'from-green-400 via-emerald-400 to-teal-400'
      : 'from-red-400 via-rose-400 to-pink-400',
    bgColor: isBullish ? 'bg-green-50' : 'bg-red-50',
    textColor: isBullish ? 'text-green-700' : 'text-red-700',
    badgeBg: isBullish ? 'from-green-500 to-emerald-500' : 'from-red-500 to-rose-500',
  }), [isBullish]);
    ...
    <div className="bg-gradient-to-br from-white to-indigo-50 rounded-xl shadow-lg p-4 border border-indigo-200 hover:shadow-xl transition-shadow">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw className={`w-3 h-3 text-indigo-600 ${isFetching ? 'animate-spin' : ''}`} />
          </button>
        </div>
      </div>
...
/**
 * Hook for using Asian market data
 */
export function useAsianMarkets() {
  return useQuery({
    queryKey: ['asian-markets'],
    queryFn: async () => await marketAPI.getAsianMarkets(),
    refetchInterval: 5 * 60 * 1000,
