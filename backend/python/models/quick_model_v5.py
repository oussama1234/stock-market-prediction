...
def main():
    ...
    Or: python quick_model_v5.py predict --features '{"current_price": 300, ...}'
    """
    if len(sys.argv) < 2:
        print(json.dumps({
    ...
    try:
        # Handle both formats: direct JSON or 'predict --features JSON'
        features_json = None
        if len(sys.argv) >= 4 and sys.argv[1] == 'predict' and sys.argv[2] == '--features':
            # Format: python script.py predict --features '{"json": "data"}'
            features_json = sys.argv[3]
        else:
            # Format: python script.py '{"json": "data"}'
