#!/usr/bin/env python3
"""
Quick Model V3 - Advanced Stock Prediction with Correction Detection
====================================================================

Advanced prediction model with comprehensive correction pattern detection,
rebound analysis, and mean reversion signals.

Features:
- Binary classification: BULLISH or BEARISH with confidence
- Asian market influence integration
- ADVANCED: 8 correction patterns detection
...
1. Parabolic rise + overbought RSI
2. Extreme deviation from moving averages
3. Volume exhaustion at highs
4. Bearish divergence (price up, indicators down)
5. Gap exhaustion
6. Extended rally + distribution signs
7. Mean reversion triggers
8. Volatility spike patterns
...
class QuickModelV3:
    """
    Quick Model V3 - Advanced prediction model with correction detection and rebound analysis
    """
    ...
    def _get_default_model_path(self):
        ...
        return str(models_dir / 'quick_model_v3.pkl')
    ...
    def predict(self, features):
        ...
        if rebound_boost > 0.3:
            # Strong rebound detected - apply stronger boost to base score
            base_score = base_score + (rebound_boost * 0.35)  # Increased rebound weight
        ...
        if correction_warning['warning']:
            ...
            if correction_direction == 'DOWN' and final_score > 0:
                ...
                # Reduce bullish confidence based on correction score
                ...
                # Flip to bullish or reduce bearish confidence
        ...
        # Generate top reasons (include correction info)
        top_reasons = self._generate_reasons(
            features, final_score_with_correction, asian_influence_score, base_score, correction_warning
        )
    ...
    def _detect_rebound_pattern(self, features):
        ...
        if news_sentiment > 0.3 and price_change_7d < 0:
            # Stock was down but has positive news - strong rebound signal
            strength = min(abs(price_change_7d) / 15, 0.4)  # More decline = stronger rebound potential
            score += 0.4 + strength  # Strong base for rebound detection
        ...
        if price_change_7d < -3 and price_change_3d > 1 and price_change_1d > 0:
            ...
            score += np.clip(recovery_strength / 8, 0, 0.5)  # Increased V-shape weight
    ...
    def _calculate_expected_move(self, final_score, atr, bb_width):
        ...
        # Calibration curve - REALISTIC daily moves
        calibration = {
            0.1: 0.3,   # Very weak signal -> 0.3% move
            0.3: 0.7,   # Weak signal -> 0.7% move
            0.5: 1.2,   # Moderate signal -> 1.2% move
            0.7: 1.8,   # Strong signal -> 1.8% move
            1.0: 2.5    # Very strong signal -> 2.5% max move
        }
        ...
        # Volatility adjustment - CONSERVATIVE (cap at 20% boost)
        volatility_factor = min(1.2, 1 + (atr / 50.0)) if atr > 0 else 1.0
    ...
    def _detect_correction_warning(self, features):
        """
        ADVANCED: Detect comprehensive correction patterns
        
        Analyzes 8 different correction patterns to identify when a stock
        is likely to correct (mean revert). Returns detailed warnings with
        severity levels and recommended actions.
        
        Returns:
            Dict with warning details or {'warning': False, 'correction_score': 0}
        """
        warnings = []
        correction_score = 0  # 0-100 scale
        ...
        ema_12 = features.get('ema_12', close)
        ema_26 = features.get('ema_26', close)
        ...
        # =================================================================
        # PATTERN 1: Parabolic Rise + Overbought RSI (CRITICAL)
        # =================================================================
        if price_change_7d > 12 and rsi_14 > 75:
            severity = 'CRITICAL' if (rsi_14 > 85 and price_change_7d > 20) else 'HIGH'
            confidence = min(95, 50 + (rsi_14 - 70) * 2 + (price_change_7d - 10))
            correction_score += 30
            
            warnings.append({
                'pattern': 'Parabolic Rise + Overbought',
                'reason': f'↗️ Parabolic rise: +{price_change_7d:.1f}% (7d) with RSI {rsi_14:.0f}',
                'severity': severity,
        ...
        # =================================================================
        # PATTERN 2: Extreme Deviation from Moving Averages
        # =================================================================
        deviation_from_ema12 = ((close - ema_12) / ema_12) * 100 if ema_12 > 0 else 0
        deviation_from_ema26 = ((close - ema_26) / ema_26) * 100 if ema_26 > 0 else 0
        
        if deviation_from_ema12 > 8 or deviation_from_ema26 > 10:
            severity = 'HIGH' if deviation_from_ema12 > 12 else 'MEDIUM'
            confidence = min(85, 40 + abs(deviation_from_ema12) * 3)
            correction_score += 20
            
            warnings.append({
                'pattern': 'Extreme Deviation from MAs',
                'reason': f'📊 Price {deviation_from_ema12:.1f}% above EMA12 (mean reversion likely)',
                'severity': severity,
                'confidence': confidence,
                'action': 'Expect pullback to moving averages'
            })
        
        # =================================================================
        # PATTERN 3: Bollinger Band Breakout Exhaustion
        # =================================================================
        if bb_pct > 0.95 and bb_width > 0:
            # Calculate how far outside bands
            z_score = (close - bb_middle) / (bb_width / 2) if bb_width > 0 else 0
            
            if z_score > 2.0:
                severity = 'CRITICAL' if z_score > 2.5 else 'HIGH'
                confidence = min(90, 50 + (z_score - 2.0) * 20)
                correction_score += 25
                
                warnings.append({
                    'pattern': 'BB Breakout Exhaustion',
                    'reason': f'🎯 Price {z_score:.1f} std devs above mean (extreme overbought)',
                    'severity': severity,
                    'confidence': confidence,
                    'action': 'Expect reversion to Bollinger middle band'
                })
        
        # =================================================================
        # PATTERN 4: Bearish Divergence (Price up, RSI/Momentum down)
        # =================================================================
        if price_change_7d > 5 and rsi_14 < rsi_7:
            # Price rising but RSI showing weakness
            divergence_strength = abs(rsi_14 - rsi_7)
            
            if divergence_strength > 5:
                severity = 'HIGH' if divergence_strength > 10 else 'MEDIUM'
                confidence = 60 + divergence_strength * 2
                correction_score += 18
                
                warnings.append({
                    'pattern': 'Bearish Divergence',
                    'reason': f'⚠️ Price rising but momentum weakening (RSI divergence)',
                    'severity': severity,
                    'confidence': confidence,
                    'action': 'Momentum loss suggests correction ahead'
                })
        
        # =================================================================
        # PATTERN 5: Volume Exhaustion at Highs
        # =================================================================
        if price_change_7d > 8 and volume_ratio < 0.8:
            # Rising price on declining volume = exhaustion
            severity = 'HIGH'
            confidence = 70
            correction_score += 15
            
            warnings.append({
                'pattern': 'Volume Exhaustion',
                'reason': f'📉 Rising price on declining volume (only {volume_ratio:.1f}x average)',
                'severity': severity,
                'confidence': confidence,
                'action': 'Lack of volume support signals weakness'
            })
        
        # =================================================================
        # PATTERN 6: Extended Rally (Too Far, Too Fast)
        # =================================================================
        if price_change_3d > 8 and price_change_7d > 15:
            severity = 'HIGH'
            confidence = 75
            correction_score += 20
            
            warnings.append({
                'pattern': 'Extended Rally',
                'reason': f'🚀 Unsustainable pace: +{price_change_3d:.1f}% (3d), +{price_change_7d:.1f}% (7d)',
                'severity': severity,
                'confidence': confidence,
                'action': 'Rapid gains rarely sustained - expect pause/pullback'
            })
        
        # =================================================================
        # PATTERN 7: Mean Reversion Trigger (Multiple Indicators)
        # =================================================================
        mean_reversion_signals = 0
        if rsi_14 > 70: mean_reversion_signals += 1
        if bb_pct > 0.85: mean_reversion_signals += 1
        if deviation_from_ema12 > 6: mean_reversion_signals += 1
        if price_change_7d > 10: mean_reversion_signals += 1
        
        if mean_reversion_signals >= 3:
            severity = 'HIGH' if mean_reversion_signals == 4 else 'MEDIUM'
            confidence = 50 + (mean_reversion_signals * 10)
            correction_score += 15
            
            warnings.append({
                'pattern': 'Mean Reversion Trigger',
                'reason': f'🎲 {mean_reversion_signals}/4 reversion signals active',
                'severity': severity,
                'confidence': confidence,
                'action': 'Multiple indicators suggest pullback imminent'
            })
        
        # =================================================================
        # PATTERN 8: Volatility Spike Pattern
        # =================================================================
        if atr > 0:
            # High volatility + overbought = correction risk
            volatility_pct = (atr / close) * 100 if close > 0 else 0
            
            if volatility_pct > 3 and rsi_14 > 65 and price_change_7d > 8:
                severity = 'MEDIUM'
                confidence = 65
                correction_score += 12
                
                warnings.append({
                    'pattern': 'Volatility Spike',
                    'reason': f'💥 High volatility ({volatility_pct:.1f}%) at extended levels',
                    'severity': severity,
                    'confidence': confidence,
                    'action': 'Increased volatility often precedes reversals'
                })
        
        # =================================================================
        # INVERSE CHECK: Oversold Correction (Upward)
        # =================================================================
        if price_change_7d < -10 and rsi_14 < 30:
            # This is actually a BUY signal - oversold correction upward expected
            correction_score = -correction_score  # Negative = bullish correction
            severity = 'HIGH'
            confidence = min(90, 50 + abs(price_change_7d))
            
            warnings = [{
                'pattern': 'Oversold - Upward Correction Due',
                'reason': f'✅ Severely oversold: {price_change_7d:.1f}% (7d) with RSI {rsi_14:.0f}',
                'severity': severity,
                'confidence': confidence,
                'action': 'BULLISH - Expect bounce/recovery from oversold levels',
                'direction': 'UP'
            }]
        ...
        correction_direction = 'UP' if correction_score < 0 else 'DOWN'
        ...
        return {
            'warning': bool(True),
            'correction_score': float(abs(correction_score)),
            'direction': str(correction_direction),
            'severity': str(max_severity),
            'pattern_count': int(len(warnings)),
            'patterns': warnings,
            'recommended_action': str(self._get_correction_recommendation(correction_direction, abs(correction_score))),
            'confidence': float(max([w.get('confidence', 50) for w in warnings])),
            'details': {
                ...
                'deviation_from_ma': float(deviation_from_ema12)
            }
        }
    
    def _get_correction_recommendation(self, direction, score):
        """Get actionable recommendation based on correction analysis"""
        if direction == 'UP':
            if score > 60:
                return "STRONG BUY - Extreme oversold, high probability bounce"
            elif score > 40:
                return "BUY - Oversold conditions favor upward correction"
            else:
                return "WATCH - Mild oversold, monitor for entry"
        else:  # DOWN
            if score > 60:
                return "TAKE PROFITS / SHORT - High correction risk"
            elif score > 40:
                return "REDUCE POSITION - Moderate correction warning"
            else:
                return "CAUTION - Minor overbought, tighten stops"
        
    
    def _generate_reasons(self, features, final_score, asian_score, base_score, correction_warning=None):
        """Generate human-readable reasons for prediction with correction alerts"""
        reasons = []
        
        # CRITICAL: Check for correction warnings FIRST
        if correction_warning and correction_warning.get('warning'):
            correction_direction = correction_warning.get('direction')
            correction_score = correction_warning.get('correction_score', 0)
            severity = correction_warning.get('severity', 'MEDIUM')
            
            # Add correction alert as first reason
            if correction_direction == 'DOWN':
                if severity == 'CRITICAL':
                    reasons.append(f"⚠️ CRITICAL CORRECTION ALERT: High risk of downward correction ({correction_score}/100 score)")
                elif severity == 'HIGH':
                    reasons.append(f"⚠️ Correction Warning: Overbought conditions suggest pullback likely")
                else:
                    reasons.append(f"⚠️ Caution: Signs of potential correction detected")
            elif correction_direction == 'UP':
                if correction_score > 60:
                    reasons.append(f"✅ BULLISH CORRECTION: Severely oversold, upward bounce expected")
                else:
                    reasons.append(f"✅ Oversold: Conditions favor upward mean reversion")
        
        # PRIORITY: Detect and report rebound patterns
        price_change_1d = features.get('price_change_1d', 0)
        price_change_3d = features.get('price_change_3d', 0)
        price_change_7d = features.get('price_change_7d', 0)
        news_sentiment = features.get('news_sentiment_score', 0)
...
def main():
    """CLI interface"""
    parser = argparse.ArgumentParser(description='Quick Model V3 - Advanced Stock Prediction with Correction Detection')
    ...
    # Initialize model
    model = QuickModelV3(model_path=args.model_path)
